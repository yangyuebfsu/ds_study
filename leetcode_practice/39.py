...
class Solution:
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        times = [target//x+1 for x in candidates]
        possibilities=[]
        for x in range(len(candidates)):
            possibilities.append(list(range(times[x])))
        attempts=np.array( list( itertools.product( *possibilities ) ) )
        candidates=np.array(candidates)
        results=attempts.dot(candidates)
        indexes=np.where(results==target)
        output=[]
        for index in attempts[indexes]:
            one=[]
            for x in range(len(candidates)):
                one=one+[candidates[x]]*index[x]
            output.append(one)
        return output
